...
COPY --from=package-builder /app/dist/mapio_gpio_ha*.whl /app/
RUN pip --python "$(which python)" install --no-cache-dir --no-deps ./mapio_gpio_ha*.whl

ENTRYPOINT [ "mapio_gpio_ha" ]
